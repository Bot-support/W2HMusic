...
async def play(_, message: Message):
    global que
    lel = await message.reply("♋ **Processing Via 🔥Talkzone🔥**")
                      ...
                      if administrator == message.from_user.id:  
                          ...
                          try:
                              ...
                              await lel.edit(
                                  ...
                                  "\n\nOr manually add @talkzoneXmusic to your Group and try again</b>",
                              )
                              pass
    try:
        chatdetails = await USER.get_chat(chid)
    ...
    await lel.edit("🔎 **Finding Via 🔥Talkzone🔥**")
    sender_id = message.from_user.id
    ...
    for i in message.command[1:]:
        query += ' ' + str(i)
    print(query)
    await lel.edit("🎵 **Processing Via 🔥Talkzone🔥**")
            ...
            [   
                ...
                [
                   InlineKeyboardButton(
                       text="⭐ Support ⭐",
                       url='https://t.me/patricia_support')
                ],
                [       
                    InlineKeyboardButton(
                        text="❌ Close",
    ...
    if message.chat.id in callsmusic.pytgcalls.active_calls:
        ...
        photo="final.png",
        reply_markup=keyboard,
        caption="▶️ **Playing** song requested by {} via Talkzone ".format(
...
async def deezer(client: Client, message_: Message):
    global que
    lel = await message_.reply("♋ **Processing Via 🔥Talkzone🔥**")
    administrators = await get_administrators(message_.chat)
    chid = message_.chat.id
    try:
        user = await USER.get_me()
    except:
        user.first_name =  "Talkzone"
    usar = user
    wew = usar.id
    try:
        #chatdetails = await USER.get_chat(chid)
    ...
    keyboard = InlineKeyboardMarkup(
         [   
             ...
             [
               InlineKeyboardButton(
                   text="✨Support✨",
                   url='https://t.me/patricia_suport')
             ],
             [       
                 InlineKeyboardButton(
                     text="❌ Close",
    ...
    if message_.chat.id in callsmusic.pytgcalls.active_calls:
        ...
        appendable = [s_name, r_by, loc]
        qeue.append(appendable)
        await res.edit_text(f"🔥Talkzone🔥= #️⃣ Queued at position {position}")
    else:
        await res.edit_text("🔥Talkzone🔥=▶️ Playing.....")
